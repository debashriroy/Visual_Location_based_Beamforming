...
from tensorflow.keras.layers import Dense,concatenate, Dropout, Conv1D, Flatten, Reshape
...
if multimodal == 2:
	...
	acc_file_name = "lidar_img"
	lidar_output_first, lidar_output_second = tf.split(value= lidar_model.output, num_or_size_splits = 2, axis = 1)
	lidar_input_first, lidar_input_second = tf.split(value= lidar_model.input, num_or_size_splits = 2, axis = 1)
        combined_model = concatenate([lidar_model.output,img_model.output])
	#np.reshape(combined_model, (combined_model[1], 1))
        #z = Dense(num_classes*2, activation="relu", use_bias=True,kernel_regularizer=regularizers.l1_l2(l1=0.01, l2=0.01),bias_regularizer=regularizers.l2(0.01),activity_regularizer=regularizers.l2(0.01))(combined_model)
	#z = Reshape((combined_model.shape[1],1))(combined_model)
	#z = Conv1D(num_classes*2, kernel_size = 1,strides= 1, activation = "relu")(z)
	#z = Dropout(0.5)(z)
	#z = Flatten()(z)	

	#z = Dropout(0.2)(z)
	z = Dense(num_classes*2, activation = "relu", use_bias=True)(combined_model)
	#z = Dense(num_classes*3, activation = "relu", use_bias=True)(z)
	#z = Dense(num_classes*2, activation = "relu", use_bias=True)(z)
	#z = Dense(num_classes*2, activation = "relu", use_bias=True,kernel_regularizer=regularizers.l1_l2(l1=0.01, l2=0.01),bias_regularizer=regularizers.l2(0.01),activity_regularizer=regularizers.l2(0.01))(z)
	z = Dense(num_classes, activation = "softmax", use_bias=True,kernel_regularizer=regularizers.l1_l2(l1=0.01, l2=0.01),bias_regularizer=regularizers.l2(0.01),activity_regularizer=regularizers.l2(0.01))(z)
        ...
        model.compile(loss=categorical_crossentropy,
                    ...
                    metrics=[metrics.categorical_accuracy,top_2_accuracy,
                            ...
                            top_50_accuracy])
        model.summary()
        hist = model.fit([X_lidar_train,X_img_train],y_train,validation_data=([X_lidar_validation, X_img_validation], y_validation), epochs=args.epochs,batch_size=args.bs, callbacks=[tf.keras.callbacks.ModelCheckpoint(filepath, monitor='val_categorical_accuracy', verbose=2, save_best_only=True, mode='auto'), tf.keras.callbacks.EarlyStopping(monitor='val_categorical_accuracy', patience=5, verbose=2, mode='auto')])

elif multimodal == 3:
    acc_file_name = "lidar_img_coord"
    ...
    model.compile(loss=categorical_crossentropy,
                ...
                metrics=[metrics.categorical_accuracy, top_2_accuracy,
                        ...
                        top_50_accuracy])
    model.summary()
    hist = model.fit([X_lidar_train,X_img_train,X_coord_train],y_train,validation_data=([X_lidar_validation, X_img_validation, X_coord_validation], y_validation),epochs=args.epochs,batch_size=args.bs, callbacks=[tf.keras.callbacks.ModelCheckpoint(filepath, monitor='val_categorical_accuracy', verbose=2, save_best_only=True, mode='auto'), tf.keras.callbacks.EarlyStopping(monitor='val_categorical_accuracy', patience=5, verbose=2, mode='auto')])
...
plt.legend()
plt.tight_layout()
fig.savefig(acc_file_name+'.png')  # save the figure to file
plt.close(fig)
