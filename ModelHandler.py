    ...
    def createArchitecture(self,model_type,num_classes,input_shape,chain,strategy,fusion):
        ...
        if(model_type == 'coord_mlp'):
            ...
            if fusion:
                ...
                # np.savetxt('coord_data.txt', out)
                print("coord features: ", out)
                # tensorflow.keras.backend.eval(tensorflow.keras.backend.print_tensor(out, message='coord out = '))
            ...
            if fusion:
                print("image features: ", out)
                #tensorflow.keras.backend.print_tensor(out, message='image out = ')
                # np.savetxt('img_data.txt', out)
            ...
            out =  layer = Conv2D(channel, (3, 3), padding="SAME", activation='relu', name='lidar_conv8')(layer) # out worked here
            ...
            if fusion :
                #out = Reshape((50, 32))(out)
                # np.savetxt('lidar_data.txt', out)
                #print("lidar features: ", out)
                #tensorflow.keras.backend.print_tensor(out, message='lidar out = ')
                print("It is a multimodal scenario involving LIDAR")
                architecture = Model(inputs=input_lid, outputs=out)
            else:
                if strategy == 'one_hot':
                    out = Dense(num_classes, activation='softmax')(layer)
                    ...
                    out = Dense(num_classes)(layer)
                print("It is a LIDAR singlemodal scenario")
                architecture = Model(inputs=input_lid, outputs=out)
