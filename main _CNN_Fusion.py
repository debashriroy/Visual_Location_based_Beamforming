...
if multimodal == 2:
    if 'coord' in args.input and 'lidar' in args.input:
        ...
        hist = model.fit(x_train, y_train,
                         ...
                         callbacks=[
                             tf.keras.callbacks.ModelCheckpoint(filepath, monitor='val_categorical_accuracy', verbose=2,
                                                                save_best_only=True, mode='auto'),
                             tf.keras.callbacks.EarlyStopping(monitor='val_categorical_accuracy', patience=5, verbose=2,
                                                              mode='auto')])
        ...
        # z = Conv1D(num_classes*2, kernel_size = 4,strides= 2)(z)
        # z = BatchNormalization()(z)
        # z= Activation('relu') (z)

        # z = Conv1D(num_classes * 1, kernel_size=2, strides=1)(z)
        # z = BatchNormalization()(z)
        # z = Activation('relu')(z)
        ...
        ####################### USING BATCH NORMALIZATION #####################
        # z = Dense(num_classes * 2, use_bias=True)(z)
        # z = BatchNormalization()(z)
        # z = Activation('relu')(z)
        z = Dropout(0.5)(z)
...
#plt.xticks(x_pos, x)
plt.tight_layout()
fig.savefig(file_name+'_pred.png')  # save the figure to file
plt.close(fig)
